...
extern "C"{
int _declspec(dllexport) plugin_is_GPL_compatible;
}
...
static void *bump_alloc(struct Bump **head, size_t len) {
	if (!*head || (*head)->capacity - (*head)->index < len) {
		...
		if (!(new_head = (Bump *)malloc(sizeof *new_head + capacity)))
			return NULL;
...
static struct EmacsStr *copy_emacs_string(emacs_env *env, struct Bump **bump, emacs_value value) {
	...
	if (!(result = (EmacsStr*)bump_alloc(bump, sizeof *result + len
...
static DWORD WINAPI worker_routine(void *ptr) {
	struct Worker *worker = (Worker*)ptr;
	...
	do {
		...
		if (pthread_mutex_lock(&shared->mutex)) return (DWORD)JOB_FAILED;//&job_failed;
		batch = shared->batches < shared->batches_end ? shared->batches++ : NULL;
		pthread_mutex_unlock(&shared->mutex);
	} while (batch);

	return (DWORD)JOB_FINISHED;//&job_finished;
}
...
emacs_value hotfuzz_filter(emacs_env *env, ptrdiff_t nargs, emacs_value args[], void *data_ptr) {
	struct Data *data = (Data*)data_ptr;
	...
	while (env->is_not_nil(env, list)) {
		if ((batches && batches[batch_idx].len >= BATCH_SIZE ? ++batch_idx : batch_idx)
			...
			if (!(new_batches = (Batch*)realloc(batches, capacity * sizeof *batches)))
				goto error;
			batches = new_batches;
	...
	for (unsigned i = 0; i < worker_count; ++i) {
		...
		res = (JobRetVal)((int)res | (int)*retval);
	}
...
extern "C" int _declspec(dllexport) emacs_module_init(struct emacs_runtime *rt) {
	...
	data.max_workers = get_CPU_core_num();// sysconf(_SC_NPROCESSORS_ONLN);
	if (!(data.workers = (Worker*)malloc(data.max_workers * sizeof *data.workers)))
		return 1;
